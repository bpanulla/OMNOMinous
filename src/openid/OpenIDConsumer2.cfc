...
<cfcomponent name="OpenIDConsumer2" extends="OpenIDConsumer" output="false" hint="Consumer library for OpenID auth framework. Supports OpenID v1 and v2.">
	...
	<cfset Variables.yadis = CreateObject("component", "OpenID.yadis").init(Variables.proxyServer, Variables.proxyPort, Variables.proxyUser, Variables.proxyPassword) />
			...
			<cfelseif StructFind(messageScope, 'openid.mode') eq "id_res">
				...
				<cfif StructKeyExists(Local.OpenID, "nonce") and StructKeyExists(Url, "nonce") and Local.OpenID['nonce'] eq URLDecode(Url['nonce'])>
					...
					<cfif StructKeyExists(messageScope, "openid.claimed_id") and StructFind(messageScope, 'openid.claimed_id') is not Local.OpenID['openid.identity']>
						...
						<cfif not StructKeyExists(Local.discoResponse, "status") or Local.discoResponse['server'] is not StructFind(messageScope, 'openid.op_endpoint')>
	...
	<cffunction name="getAssociate" returntype="struct" access="private" output="false" hint="Establish a 'DH-SHA1' association and shared secret between Consumer and OP.">
		...
		<cfhttp method="post" url="#Arguments.Request['openid_server']#" result="Local.cfhttp" proxyServer="#Variables.proxyServer#" proxyPort="#Variables.proxyPort#" proxyUser="#Variables.proxyUser#" proxyPassword="#Variables.proxyPassword#">
			<cfif StructKeyExists(Arguments.Request, "openid.ns")>
				<cfhttpparam type="formfield" name="openid.ns" value="#UrlEncodedFormat(Arguments.Request['openid.ns'])#" encoded="false" />
			</cfif>
			...
			<cfhttpparam type="formfield" name="openid.dh_consumer_public" value="#UrlEncodedFormat(Local.cpub)#" encoded="false" />
		</cfhttp>
	...
	<cffunction name="isValidHandle" returntype="boolean" access="private" output="false" hint="Ask OP if a message is valid">
		...
		<cfhttp method="post" url="#Arguments.Request['openid_server']#" result="Local.cfhttp" proxyServer="#Variables.proxyServer#" proxyPort="#Variables.proxyPort#" proxyUser="#Variables.proxyUser#" proxyPassword="#Variables.proxyPassword#">
			...
			<cfhttpparam type="formfield" name="openid.assoc_handle" value="#UrlEncodedFormat(Arguments.Request['assoc_handle'])#" encoded="false" />
			<cfhttpparam type="formfield" name="openid.sig" value="#UrlEncodedFormat(StructFind(messageScope, 'openid.sig'))#" encoded="false" />
			<cfhttpparam type="formfield" name="openid.signed" value="#UrlEncodedFormat(StructFind(messageScope, 'openid.signed'))#" encoded="false" />
			<cfloop index="Local.sKey" list="#StructFind(messageScope, 'openid.signed')#">
				<cfif Local.sKey neq "mode">
					<cfhttpparam type="formfield" name="openid.#LCase(Local.sKey)#" value="#UrlEncodedFormat(StructFind(messageScope, 'openid.' & Local.sKey))#" encoded="false" />
				</cfif>
			</cfloop>
		</cfhttp>
	...
	<cffunction name="htmlServerDiscovery" returntype="struct" access="private" output="false" hint="HTML-based discovery of OpenID servers.">
		...
		<cfhttp method="get" url="#Arguments.identifier#" result="Local.cfhttp" proxyServer="#Variables.proxyServer#" proxyPort="#Variables.proxyPort#" proxyUser="#Variables.proxyUser#" proxyPassword="#Variables.proxyPassword#" timeout="10" />
	...
	<cfset Request.opIdentifierFilter = Variables.opIdentifierFilter />
	...
	<cfset Request.openID1Filter = Variables.openID1Filter />
